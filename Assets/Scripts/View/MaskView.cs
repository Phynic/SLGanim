...
public class MaskView : ViewBase<MaskView>
{
    ...
    public void FadeIn()
    {
        ...
        transform.SetAsLastSibling();
        var tween = fadeImage.DOColor(new Color(0, 0, 0, 0), GameManager.GetInstance().fadeTime);
    ...
    public void FadeOut(bool setAsLastSibling = true, TweenCallback onComplete = null)
    {
        ...
        if (setAsLastSibling)
            transform.SetAsLastSibling();
        fadeImage.DOColor(new Color(0, 0, 0, 1), GameManager.GetInstance().fadeTime).onComplete = onComplete;
        base.Open();
    }
}
